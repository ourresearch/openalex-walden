#!/usr/bin/env python3
"""
DOE (Department of Energy) Awards to S3 Data Pipeline
======================================================

This script downloads DOE grant data from USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/doe/doe_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for DOE grants (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/doe/

Usage:
    python doe_to_s3.py

    # Resume interrupted download:
    python doe_to_s3.py --resume

    # Skip upload to S3:
    python doe_to_s3.py --skip-upload
...
# DOE agency info
DOE_AGENCY_NAME = "Department of Energy"
...
S3_KEY = "awards/doe/doe_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for DOE grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "doe_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download DOE grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"doe_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download DOE grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading DOE grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {DOE_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-DOE-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("doe_fy*.zip"))
...
def save_to_parquet(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    Returns:
        Path to output file
    """
    output_path = output_dir / "doe_awards.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download DOE grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./doe_data"),
        help="Directory for downloaded/processed files (default: ./doe_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("DOE Awards to S3 Data Pipeline")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("doe_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateDOEAwards.ipynb")
