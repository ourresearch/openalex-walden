#!/usr/bin/env python3
"""
Congressionally Directed Medical Research Programs (CDMRP) awards.

Source: USAspending.gov (Method 2/4, public API). CDMRP is NOT cleanly exposed as
an awarding sub-tier (DoD/Army office fields are null on assistance awards), so we
key off CDMRP's FAIN prefix instead: a keyword search on `W81XWH` (CDMRP's award-id
prefix) within assistance awards, keeping only award ids that actually start with
`W81XWH`. CDMRP's grant numbers (W81XWH-...) are heavily cited in papers, so the
...
USAspending caps paginated search at ~10k records, and CDMRP has ~16.7k grants, so
we split the pull by award sign-year (each year << 10k) and de-dup by award id.

Awarding body funder: F4320338273 (Congressionally Directed Medical Research Programs, US).
provenance = 'usaspending_cdmrp' (aggregator-suffixed per runbook).
"""
import sys
...
FUNDER_ID = 4320338273          # Congressionally Directed Medical Research Programs, US
PROVENANCE = "usaspending_cdmrp"
FAIN_PREFIX = "W81XWH"          # CDMRP award-id prefix
COUNTRY = "US"
CURRENCY = "USD"
S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/cdmrp/cdmrp_projects.parquet"
...
def fetch_year(year: int) -> list:
    """All W81XWH assistance awards signed in the given year (paginated)."""
    out, page = [], 1
...
def scrape(limit: Optional[int], max_year: Optional[int]) -> pd.DataFrame:
    log("=" * 60)
    log("Step 1: harvest CDMRP (W81XWH) assistance awards from USAspending")
    log("=" * 60)
    ...
    for year in range(FIRST_YEAR, end_year + 1):
        ...
        for r in rows:
            ...
            if not aid.upper().startswith(FAIN_PREFIX):   # exclude other Army offices (N62909..., etc.)
                continue
            if aid not in by_id:
                by_id[aid] = r
                kept += 1
        log(f"  {year}: {len(rows)} rows, +{kept} new CDMRP (total {len(by_id):,})")
        if limit and len(by_id) >= limit:
            break
    records = list(by_id.values())
    if limit:
        records = records[:limit]
    log(f"  harvested {len(records):,} distinct CDMRP awards")
    ...
    if df.empty:
        raise RuntimeError("No CDMRP awards harvested — aborting.")
...
def main():
    ap = argparse.ArgumentParser(description="CDMRP (USAspending W81XWH) awards -> parquet -> S3")
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/cdmrp"))
    ...
    out = args.output_dir / "cdmrp_projects.parquet"
