#!/usr/bin/env python3
"""
ACF Awards to S3 Data Pipeline (US Administration for Children and Families, HHS subtier)
================================

This script downloads ACF grant data from USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/acf/acf_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for ACF grants (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/acf/

Usage:
    python acf_to_s3.py

    # Resume interrupted download:
    python acf_to_s3.py --resume

    # Skip upload to S3:
    python acf_to_s3.py --skip-upload
...
# ACF agency info
ACF_AGENCY_NAME = "Administration for Children and Families"
...
S3_KEY = "awards/acf/acf_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for ACF grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "acf_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download ACF grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"acf_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download ACF grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading ACF grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {ACF_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-ACF-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("acf_fy*.zip"))
...
def save_to_parquet(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    Returns:
        Path to output file
    """
    output_path = output_dir / "acf_awards.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download ACF grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./acf_data"),
        help="Directory for downloaded/processed files (default: ./acf_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("ACF Awards to S3 Data Pipeline (US Administration for Children and Families, HHS subtier)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("acf_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateACFAwards.ipynb")
