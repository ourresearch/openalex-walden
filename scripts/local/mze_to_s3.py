#!/usr/bin/env python3
"""
Czech Ministry of Agriculture (MZE) to S3 Data Pipeline
====================================================

This script downloads Czech Ministry of Agriculture (MZE) grants data from the IS VaVaI
...
- We filter for provider code "MZE" (Ministry of Agriculture)

Output: s3://openalex-ingest/awards/mze/mze_projects.parquet

What this script does:
1. Downloads CEP-projekty.csv and CEP-ucastnici.csv from IS VaVaI
2. Filters projects where poskytovatel = "MZE" (Ministry of Agriculture)
...
Requirements:
    ...
    s3://openalex-ingest/awards/mze/

Usage:
    python mze_to_s3.py

    # Skip S3 upload (for testing):
    python mze_to_s3.py --skip-upload
...
# MZE provider code in IS VaVaI
MZE_PROVIDER_CODE = "MZE"

# S3 destination
S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/mze/mze_projects.parquet"
...
def process_mze_data(projects_df: pd.DataFrame, participants_df: pd.DataFrame) -> pd.DataFrame:
    """Filter for MZE projects and enrich with participant data."""
    print("\nProcessing MZE data...")

    # Filter for MZE projects
    mze_projects = projects_df[projects_df['poskytovatel'] == MZE_PROVIDER_CODE].copy()
    print(f"  MZE projects: {len(mze_projects):,} (of {len(projects_df):,} total)")

    # Filter participants for MZE projects
    mze_participants = participants_df[participants_df['poskytovatel'] == MZE_PROVIDER_CODE].copy()
    print(f"  MZE participants: {len(mze_participants):,}")
    ...
    participant_ror = mze_participants[['kod_projektu', 'ucastnik', 'ROR', 'nazev_ucastnika']].drop_duplicates()
    ...
    mze_projects = mze_projects.merge(
        participant_ror,
    ...
    columns_to_keep = [c for c in columns_to_keep if c in mze_projects.columns]
    mze_projects = mze_projects[columns_to_keep]

    # Rename columns to English
    mze_projects = mze_projects.rename(columns={
    ...
    for col in ['total_cost', 'funding_amount', 'foreign_funding']:
        if col in mze_projects.columns:
            mze_projects[col] = pd.to_numeric(mze_projects[col], errors='coerce')

    # Use English title if available, fall back to Czech
    mze_projects['title'] = mze_projects['title_en'].fillna(mze_projects['title_cs'])
    mze_projects['description'] = mze_projects['description_en'].fillna(mze_projects['description_cs'])

    print(f"\nFinal dataset: {len(mze_projects):,} projects")
...
def main():
    parser = argparse.ArgumentParser(description="Download MZE projects from IS VaVaI to S3")
    parser.add_argument("--output-dir", type=Path, default=Path("/tmp/mze_download"),
    ...
    print("=" * 60)
    print("Czech Ministry of Agriculture (MZE) Data Download")
    print("=" * 60)
    ...
    # Process MZE data
    mze_df = process_mze_data(projects_df, participants_df)

    if mze_df.empty:
        print("No MZE projects found!")
        return 1

    # Save to parquet
    output_path = args.output_dir / "mze_projects.parquet"
    mze_df.to_parquet(output_path, index=False)
    print(f"\nSaved to {output_path}")
    print(f"  Rows: {len(mze_df):,}")
    print(f"  Columns: {list(mze_df.columns)}")

    # Show sample
    print("\nSample data:")
    sample_cols = ['project_code', 'title', 'funding_amount', 'start_year']
    sample_cols = [c for c in sample_cols if c in mze_df.columns]
    print(mze_df[sample_cols].head(3).to_string())

    # Summary stats
    print("\n" + "=" * 60)
    print("Summary Statistics")
    print("=" * 60)
    print(f"Total projects: {len(mze_df):,}")

    if 'funding_amount' in mze_df.columns:
        has_amount = mze_df['funding_amount'].notna().sum()
        print(f"Projects with funding amount: {has_amount:,}")
        if mze_df['funding_amount'].notna().any():
            total_funding = mze_df['funding_amount'].sum()
            print(f"Total funding: CZK {total_funding:,.0f} (~EUR {total_funding/25:,.0f})")

    if 'start_year' in mze_df.columns:
        print(f"Year range: {mze_df['start_year'].min()} to {mze_df['start_year'].max()}")

    if 'ror_id' in mze_df.columns:
        has_ror = mze_df['ror_id'].notna().sum()
        pct_ror = has_ror / len(mze_df) * 100
