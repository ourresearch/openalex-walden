#!/usr/bin/env python3
"""
CDC Awards to S3 Data Pipeline (US Centers for Disease Control and Prevention, HHS subtier)
================================

This script downloads CDC grant data from USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/cdc/cdc_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for CDC grants (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/cdc/

Usage:
    python cdc_to_s3.py

    # Resume interrupted download:
    python cdc_to_s3.py --resume

    # Skip upload to S3:
    python cdc_to_s3.py --skip-upload
...
# CDC agency info
CDC_AGENCY_NAME = "Centers for Disease Control and Prevention"
...
S3_KEY = "awards/cdc/cdc_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for CDC grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "cdc_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download CDC grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"cdc_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download CDC grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading CDC grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {CDC_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-CDC-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("cdc_fy*.zip"))
...
def save_to_parquet(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    Returns:
        Path to output file
    """
    output_path = output_dir / "cdc_awards.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download CDC grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./cdc_data"),
        help="Directory for downloaded/processed files (default: ./cdc_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("CDC Awards to S3 Data Pipeline (US Centers for Disease Control and Prevention, HHS subtier)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("cdc_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateCDCAwards.ipynb")
