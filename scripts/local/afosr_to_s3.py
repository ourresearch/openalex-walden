#!/usr/bin/env python3
"""
Air Force Office of Scientific Research (AFOSR) awards.

Source: USAspending.gov (Method 2/4, public API). AFOSR is NOT cleanly exposed as
an awarding sub-tier (DoD/Air Force office fields are null on assistance awards), so we
key off AFOSR's FAIN prefix instead: a keyword search on `FA9550` (AFOSR's award-id
prefix) within assistance awards, keeping only award ids that actually start with
`FA9550`. AFOSR's grant numbers (FA9550-...) are heavily cited in papers, so the
...
USAspending caps paginated search at ~10k records, and AFOSR has ~16.7k grants, so
we split the pull by award sign-year (each year << 10k) and de-dup by award id.

Awarding body funder: F4320338279 (Air Force Office of Scientific Research, US).
provenance = 'usaspending_afosr' (aggregator-suffixed per runbook).
"""
import sys
...
FUNDER_ID = 4320338279          # Air Force Office of Scientific Research, US
PROVENANCE = "usaspending_afosr"
FAIN_PREFIX = "FA9550"          # AFOSR award-id prefix
COUNTRY = "US"
CURRENCY = "USD"
S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/afosr/afosr_projects.parquet"
...
def fetch_year(year: int) -> list:
    """All FA9550 assistance awards signed in the given year (paginated)."""
    out, page = [], 1
...
def scrape(limit: Optional[int], max_year: Optional[int]) -> pd.DataFrame:
    log("=" * 60)
    log("Step 1: harvest AFOSR (FA9550) assistance awards from USAspending")
    log("=" * 60)
    ...
    for year in range(FIRST_YEAR, end_year + 1):
        ...
        for r in rows:
            ...
            if not aid.upper().startswith(FAIN_PREFIX):   # exclude other Air Force offices (N62909..., etc.)
                continue
            if aid not in by_id:
                by_id[aid] = r
                kept += 1
        log(f"  {year}: {len(rows)} rows, +{kept} new AFOSR (total {len(by_id):,})")
        if limit and len(by_id) >= limit:
            break
    records = list(by_id.values())
    if limit:
        records = records[:limit]
    log(f"  harvested {len(records):,} distinct AFOSR awards")
    ...
    if df.empty:
        raise RuntimeError("No AFOSR awards harvested — aborting.")
...
def main():
    ap = argparse.ArgumentParser(description="AFOSR (USAspending FA9550) awards -> parquet -> S3")
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/afosr"))
    ...
    out = args.output_dir / "afosr_projects.parquet"
