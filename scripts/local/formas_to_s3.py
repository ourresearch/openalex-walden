#!/usr/bin/env python3
"""
Formas (Swedish Research Council for Sustainable Development) to S3 Data Pipeline
==================================================================================

This script downloads Formas grant data from SweCRIS,
...
Output: s3://openalex-ingest/awards/formas/formas_projects.parquet

What this script does:
1. Downloads Formas grants from SweCRIS API
...
Requirements:
    ...
    s3://openalex-ingest/awards/formas/

Usage:
    python formas_to_s3.py

    # Or with options:
    python formas_to_s3.py --output-dir /path/to/output --skip-upload
...
S3_KEY = "awards/formas/formas_projects.parquet"
...
def download_grants_from_api(output_dir: Path) -> Path:
    """
    Download Formas grants from SweCRIS API.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading Formas grants from SweCRIS API")
    print(f"{'='*60}")
    ...
    output_path = output_dir / "formas_grants_raw.json"

    # Download with streaming for large response
    print("  [INFO] Downloading (this may take 1-2 minutes)...")
...
def process_projects(projects: list[dict], output_dir: Path) -> Path:
    ...
    # Save to parquet
    output_path = output_dir / "formas_projects.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download Formas grants and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./formas_data"),
        help="Directory for downloaded/processed files (default: ./formas_data)"
    )
    parser.add_argument(
        "--skip-download",
        action="store_true",
    ...
    print("=" * 60)
    print("Formas (Swedish Research Council for Sustainable Development)")
    print("to S3 Pipeline")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        json_path = args.output_dir / "formas_grants_raw.json"
    ...
    if not args.skip_upload:
        ...
        if not upload_success:
            ...
            print(f"  AWS_PROFILE=openalex aws s3 cp {parquet_path} s3://{S3_BUCKET}/{S3_KEY}")
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateFormasAwards.ipynb")
