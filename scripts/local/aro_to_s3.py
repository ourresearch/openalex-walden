#!/usr/bin/env python3
"""
Army Research Office (ARO) awards.

Source: USAspending.gov (Method 2/4, public API). ARO is NOT cleanly exposed as
an awarding sub-tier (DoD/Army office fields are null on assistance awards), so we
key off ARO's FAIN prefix instead: a keyword search on `W911NF` (ARO's award-id
prefix) within assistance awards, keeping only award ids that actually start with
`W911NF`. ARO's grant numbers (W911NF-...) are heavily cited in papers, so the
...
USAspending caps paginated search at ~10k records, and ARO has ~16.7k grants, so
we split the pull by award sign-year (each year << 10k) and de-dup by award id.

Awarding body funder: F4320338281 (Army Research Office, US).
provenance = 'usaspending_aro' (aggregator-suffixed per runbook).
"""
import sys
...
FUNDER_ID = 4320338281          # Army Research Office, US
PROVENANCE = "usaspending_aro"
FAIN_PREFIX = "W911NF"          # ARO award-id prefix
COUNTRY = "US"
CURRENCY = "USD"
S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/aro/aro_projects.parquet"
...
def fetch_year(year: int) -> list:
    """All W911NF assistance awards signed in the given year (paginated)."""
    out, page = [], 1
...
def scrape(limit: Optional[int], max_year: Optional[int]) -> pd.DataFrame:
    log("=" * 60)
    log("Step 1: harvest ARO (W911NF) assistance awards from USAspending")
    log("=" * 60)
    ...
    for year in range(FIRST_YEAR, end_year + 1):
        ...
        for r in rows:
            ...
            if aid not in by_id:
                by_id[aid] = r
                kept += 1
        log(f"  {year}: {len(rows)} rows, +{kept} new ARO (total {len(by_id):,})")
        if limit and len(by_id) >= limit:
            break
    records = list(by_id.values())
    if limit:
        records = records[:limit]
    log(f"  harvested {len(records):,} distinct ARO awards")
    ...
    if df.empty:
        raise RuntimeError("No ARO awards harvested — aborting.")
...
def main():
    ap = argparse.ArgumentParser(description="ARO (USAspending W911NF) awards -> parquet -> S3")
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/aro"))
    ...
    out = args.output_dir / "aro_projects.parquet"
