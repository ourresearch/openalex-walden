#!/usr/bin/env python3
"""
NIST Awards to S3 Data Pipeline (National Institute of Standards and Technology, subtier)
================================

This script downloads NIST grant data from the USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Scope note:
    This script is for the NIST subtier agency scope, matching the existing
    AHRQ/FDA/HRSA/USGS federal-USAspending pattern for subtiers. NIST is a
    Department of Commerce subtier in USAspending, not a toptier agency.

CHECK FIRST outcome:
    NIST publishes program-specific funding pages and award announcements
    (notably SBIR), but no broad official bulk export was identified that
    covers all NIST external grants/cooperative agreements. A USAspending
    FY2024 smoke test on 2026-05-16 returned 1,063 grant/cooperative-agreement
    awards for awarding subtier "National Institute of Standards and
    Technology", including BEAD broadband grants plus SBIR/cooperative awards.
    USAspending is the broadest working source found for this NIST funder scope.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/nist/nist_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for NIST grants (subtier scope) (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/nist/

Usage:
    python nist_to_s3.py

    # Resume interrupted download:
    python nist_to_s3.py --resume

    # Skip upload to S3:
    python nist_to_s3.py --skip-upload
...
# NIST agency info
NIST_AGENCY_NAME = "National Institute of Standards and Technology"
...
S3_KEY = "awards/nist/nist_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for NIST subtier grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "nist_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download NIST grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"nist_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download NIST grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading NIST grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {NIST_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-NIST-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("nist_fy*.zip"))
...
def save_to_parquet(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    Returns:
        Path to output file
    """
    output_path = output_dir / "nist_awards.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download NIST grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./nist_data"),
        help="Directory for downloaded/processed files (default: ./nist_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("NIST Awards to S3 Data Pipeline (National Institute of Standards and Technology, subtier)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("nist_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateNISTAwards.ipynb")
