#!/usr/bin/env python3
"""
NOAA Awards to S3 Data Pipeline (National Oceanic and Atmospheric Administration, subtier)
================================

This script downloads NOAA grant data from the USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Scope note:
    This script is for the NOAA subtier agency scope, matching the existing
    AHRQ/FDA/HRSA/USGS federal-USAspending pattern for subtiers. NOAA is a
    Department of Commerce subtier in USAspending, not a toptier agency.

CHECK FIRST outcome:
    NOAA Grants Online is not reachable from the public internet,
    Climate Program Office pages are CloudFront-WAF challenged, Sea Grant's
    WordPress REST API returns 401, and no broad structured index was found on
    research.noaa.gov or data.noaa.gov. A USAspending FY2024 smoke test on
    2026-05-19 returned real grant/cooperative-agreement awards for awarding
    subtier "National Oceanic and Atmospheric Administration". USAspending is
    the broadest working source found for this NOAA funder scope.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/noaa/noaa_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for NOAA grants (subtier scope) (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/noaa/

Usage:
    python noaa_to_s3.py

    # Resume interrupted download:
    python noaa_to_s3.py --resume

    # Skip upload to S3:
    python noaa_to_s3.py --skip-upload
...
# NOAA agency info
NOAA_AGENCY_NAME = "National Oceanic and Atmospheric Administration"
...
S3_KEY = "awards/noaa/noaa_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for NOAA subtier grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "noaa_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download NOAA grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"noaa_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download NOAA grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading NOAA grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {NOAA_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-NOAA-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("noaa_fy*.zip"))
...
def save_to_parquet(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    Returns:
        Path to output file
    """
    output_path = output_dir / "noaa_awards.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download NOAA grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./noaa_data"),
        help="Directory for downloaded/processed files (default: ./noaa_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("NOAA Awards to S3 Data Pipeline (National Oceanic and Atmospheric Administration, subtier)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("noaa_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateNOAAAwards.ipynb")
