#!/usr/bin/env python3
"""
Defense Threat Reduction Agency (DTRA) awards.

Source: USAspending.gov (Method 2/4, public API). DTRA is NOT cleanly exposed as
an awarding sub-tier (DoD/DoD office fields are null on assistance awards), so we
key off DTRA's FAIN prefix instead: a keyword search on `HDTRA` (DTRA's award-id
prefix) within assistance awards, keeping only award ids that actually start with
`HDTRA`. DTRA's grant numbers (HDTRA-...) are heavily cited in papers, so the
...
USAspending caps paginated search at ~10k records, and DTRA has ~16.7k grants, so
we split the pull by award sign-year (each year << 10k) and de-dup by award id.

Awarding body funder: F4320332186 (Defense Threat Reduction Agency, US).
provenance = 'usaspending_dtra' (aggregator-suffixed per runbook).
"""
import sys
...
FUNDER_ID = 4320332186          # Defense Threat Reduction Agency, US
PROVENANCE = "usaspending_dtra"
FAIN_PREFIX = "HDTRA"          # DTRA award-id prefix
COUNTRY = "US"
CURRENCY = "USD"
S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/dtra/dtra_projects.parquet"
...
def fetch_year(year: int) -> list:
    """All HDTRA assistance awards signed in the given year (paginated)."""
    out, page = [], 1
...
def scrape(limit: Optional[int], max_year: Optional[int]) -> pd.DataFrame:
    log("=" * 60)
    log("Step 1: harvest DTRA (HDTRA) assistance awards from USAspending")
    log("=" * 60)
    ...
    for year in range(FIRST_YEAR, end_year + 1):
        ...
        for r in rows:
            ...
            if not aid.upper().startswith(FAIN_PREFIX):   # exclude other DoD offices (N62909..., etc.)
                continue
            if aid not in by_id:
                by_id[aid] = r
                kept += 1
        log(f"  {year}: {len(rows)} rows, +{kept} new DTRA (total {len(by_id):,})")
        if limit and len(by_id) >= limit:
            break
    records = list(by_id.values())
    if limit:
        records = records[:limit]
    log(f"  harvested {len(records):,} distinct DTRA awards")
    ...
    if df.empty:
        raise RuntimeError("No DTRA awards harvested — aborting.")
...
def main():
    ap = argparse.ArgumentParser(description="DTRA (USAspending HDTRA) awards -> parquet -> S3")
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/dtra"))
    ...
    out = args.output_dir / "dtra_projects.parquet"
