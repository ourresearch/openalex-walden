#!/usr/bin/env python3
"""
CSA (Canadian Space Agency) to S3 Data Pipeline
================================================
...
Data Source:
  ...
  Filter: {"owner_org": "csa-asc"} → ~1,931 CSA grant records

Output: s3://openalex-ingest/awards/csa_canada/csa_canada_projects.parquet
...
We hard-filter to owner_org='csa-asc' to avoid double-counting.

Usage:
    python csa_canada_to_s3.py --skip-upload
"""
...
def transform(rows):
    ...
    agg["provenance"] = "csa_canada_proactive"
...
def main():
    ...
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/csa_canada"))
    ap.add_argument("--skip-upload", action="store_true")
    ap.add_argument("--allow-shrink", action="store_true")
    a = ap.parse_args()
    a.output_dir.mkdir(parents=True, exist_ok=True)
    print("="*60); print("CSA (Canadian Space Agency) -> S3"); print("="*60)
    print(f"[1/2] Fetching CSA grants from Proactive Disclosure (owner_org={OWNER_ORG})")
    raw = fetch_all()
    print(f"      total rows: {len(raw):,}")
    print(f"[2/2] Aggregating by ref_number")
    df = transform(raw)
    out = a.output_dir / "csa_canada_projects.parquet"
    ...
    print(f"\nNext: notebooks/awards/CreateCSACanadaAwards.ipynb")
