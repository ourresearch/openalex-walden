#!/usr/bin/env python3
"""
Czech Ministry of Industry and Trade (MPO) to S3 Data Pipeline
====================================================

This script downloads Czech Ministry of Industry and Trade (MPO) grants data from the IS VaVaI
...
- We filter for provider code "MPO" (Czech Ministry of Industry and Trade)

Output: s3://openalex-ingest/awards/mpo/mpo_projects.parquet

What this script does:
1. Downloads CEP-projekty.csv and CEP-ucastnici.csv from IS VaVaI
2. Filters projects where poskytovatel = "MPO" (Czech Ministry of Industry and Trade)
...
Requirements:
    ...
    s3://openalex-ingest/awards/mpo/

Usage:
    python mpo_to_s3.py

    # Skip S3 upload (for testing):
    python mpo_to_s3.py --skip-upload
...
# MPO provider code in IS VaVaI
MPO_PROVIDER_CODE = "MPO"

# S3 destination
S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/mpo/mpo_projects.parquet"
...
def process_mpo_data(projects_df: pd.DataFrame, participants_df: pd.DataFrame) -> pd.DataFrame:
    """Filter for MPO projects and enrich with participant data."""
    print("\nProcessing MPO data...")

    # Filter for MPO projects
    mpo_projects = projects_df[projects_df['poskytovatel'] == MPO_PROVIDER_CODE].copy()
    print(f"  MPO projects: {len(mpo_projects):,} (of {len(projects_df):,} total)")

    # Filter participants for MPO projects
    mpo_participants = participants_df[participants_df['poskytovatel'] == MPO_PROVIDER_CODE].copy()
    print(f"  MPO participants: {len(mpo_participants):,}")
    ...
    participant_ror = mpo_participants[['kod_projektu', 'ucastnik', 'ROR', 'nazev_ucastnika']].drop_duplicates()
    ...
    mpo_projects = mpo_projects.merge(
        participant_ror,
    ...
    columns_to_keep = [c for c in columns_to_keep if c in mpo_projects.columns]
    mpo_projects = mpo_projects[columns_to_keep]

    # Rename columns to English
    mpo_projects = mpo_projects.rename(columns={
    ...
    for col in ['total_cost', 'funding_amount', 'foreign_funding']:
        if col in mpo_projects.columns:
            mpo_projects[col] = pd.to_numeric(mpo_projects[col], errors='coerce')

    # Use English title if available, fall back to Czech
    mpo_projects['title'] = mpo_projects['title_en'].fillna(mpo_projects['title_cs'])
    mpo_projects['description'] = mpo_projects['description_en'].fillna(mpo_projects['description_cs'])

    print(f"\nFinal dataset: {len(mpo_projects):,} projects")
...
def main():
    parser = argparse.ArgumentParser(description="Download MPO projects from IS VaVaI to S3")
    parser.add_argument("--output-dir", type=Path, default=Path("/tmp/mpo_download"),
    ...
    print("=" * 60)
    print("Czech Ministry of Industry and Trade (MPO) Data Download")
    print("=" * 60)
    ...
    # Process MPO data
    mpo_df = process_mpo_data(projects_df, participants_df)

    if mpo_df.empty:
        print("No MPO projects found!")
        return 1

    # Save to parquet
    output_path = args.output_dir / "mpo_projects.parquet"
    mpo_df.to_parquet(output_path, index=False)
    print(f"\nSaved to {output_path}")
    print(f"  Rows: {len(mpo_df):,}")
    print(f"  Columns: {list(mpo_df.columns)}")

    # Show sample
    print("\nSample data:")
    sample_cols = ['project_code', 'title', 'funding_amount', 'start_year']
    sample_cols = [c for c in sample_cols if c in mpo_df.columns]
    print(mpo_df[sample_cols].head(3).to_string())

    # Summary stats
    print("\n" + "=" * 60)
    print("Summary Statistics")
    print("=" * 60)
    print(f"Total projects: {len(mpo_df):,}")

    if 'funding_amount' in mpo_df.columns:
        has_amount = mpo_df['funding_amount'].notna().sum()
        print(f"Projects with funding amount: {has_amount:,}")
        if mpo_df['funding_amount'].notna().any():
            total_funding = mpo_df['funding_amount'].sum()
            print(f"Total funding: CZK {total_funding:,.0f} (~EUR {total_funding/25:,.0f})")

    if 'start_year' in mpo_df.columns:
        print(f"Year range: {mpo_df['start_year'].min()} to {mpo_df['start_year'].max()}")

    if 'ror_id' in mpo_df.columns:
        has_ror = mpo_df['ror_id'].notna().sum()
        pct_ror = has_ror / len(mpo_df) * 100
