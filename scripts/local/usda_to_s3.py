...
Requirements:
    ...
    s3://openalex-ingest/awards/usda/

Usage:
    python usda_to_s3.py

    # Resume interrupted download:
    python usda_to_s3.py --resume

    # Skip upload to S3:
    python usda_to_s3.py --skip-upload
...
# USDA agency info
USDA_AGENCY_NAME = "Department of Agriculture"
...
S3_KEY = "awards/usda/usda_awards.parquet"
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "usda_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download USDA grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"usda_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download USDA grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading USDA grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {USDA_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-USDA-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("usda_fy*.zip"))
...
def main():
    parser = argparse.ArgumentParser(
        description="Download USDA grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./usda_data"),
        help="Directory for downloaded/processed files (default: ./usda_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("USDA Awards to S3 Data Pipeline (US Department of Agriculture, toptier)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("usda_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateUSDAAwards.ipynb")
