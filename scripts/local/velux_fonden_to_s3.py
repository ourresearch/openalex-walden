#!/usr/bin/env python3
"""
Velux Fonden (Denmark) to S3 Data Pipeline
============================================

Downloads the Velux/Villum Foundations' public project database from
...
Output: s3://openalex-ingest/awards/velux_fonden/velux_fonden_projects.parquet
...
Output columns:
  funder_award_id  → `velux-fonden-{project_id}` (the foundation has no public
...
Usage:
    python velux_fonden_to_s3.py --skip-upload
"""
...
S3_KEY    = "awards/velux_fonden/velux_fonden_projects.parquet"
DATA_URL  = "https://veluxfonden.dk/en/data.js"
FOUNDATION_ID_FILTER = 58   # Villum Foundation (Velux Foundation = 58, separate ingest)

def fetch_data(output_dir: Path) -> dict:
    print(f"[1/2] Downloading veluxfonden.dk/en/data.js")
    raw_path = output_dir / "velux_data.js"
    req = urllib.request.Request(DATA_URL, headers={"User-Agent": "OpenAlex-Velux-Fonden/1.0 (contact@openalex.org)"})
...
def transform(d: dict) -> pd.DataFrame:
    ...
    for pid, p in d["project"].items():
        ...
        if pa_name and not _looks_like_org_name(pa_name):
            ...
            if len(tokens) >= 2:
                ...
                family_name = pa_name
        recs.append({
            "funder_award_id":  f"velux-fonden-{p.get('id')}",
    ...
    df["provenance"] = "velux_fonden_veluxfonden"
...
def main():
    ap = argparse.ArgumentParser(description="Velux Fonden (veluxfonden.dk/data.js) -> S3")
    ...
    print("=" * 60)
    print("Velux Fonden (Denmark) -> S3")
    print("=" * 60)
    raw = fetch_data(a.output_dir)
    df = transform(raw)
    out = a.output_dir / "velux_fonden_projects.parquet"
