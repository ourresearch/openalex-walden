#!/usr/bin/env python3
"""
FWO (Research Foundation - Flanders / Fonds Wetenschappelijk Onderzoek) awards.
...
S3_KEY = "awards/fwo/fwo_projects.parquet"
PAGE_SIZE = 100
...
def fwo_grant_id(project: ET.Element) -> Optional[str]:
    ...
    for fi in fis.findall("fundingIdentifier"):
        ...
        if not val:
            continue
        if "FWO" in (fi.get("authority") or "").upper():
            return val
...
def main() -> None:
    ...
    out = args.output_dir / "fwo_projects.parquet"
