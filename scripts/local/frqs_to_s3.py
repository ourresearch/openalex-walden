#!/usr/bin/env python3
"""
FRQS (Fonds de recherche du Québec - Santé) to S3 Data Pipeline
==================================================================================

Downloads the official FRQS funded-projects CSVs from donneesquebec.ca CKAN and
writes a parquet for Databricks.

Data Source:
  CKAN: https://www.donneesquebec.ca/recherche/api/3/action/package_show?id=liste-du-financement-accorde-par-le-fonds-de-recherche-du-quebec-sante
...
Output: s3://openalex-ingest/awards/frqs/frqs_projects.parquet

Schema notes:
  - funder_award_id = Dossier (the FRQS internal project ID, e.g. 196577 — this
    is the integer cited in OpenAlex grants links to FRQS and matches the
    'Numéro de dossier' shown on FRQS award letters).
...
Usage:
    python frqs_to_s3.py --skip-upload
"""
...
S3_KEY    = "awards/frqs/frqs_projects.parquet"
DATASET_SLUG = "liste-du-financement-accorde-par-le-fonds-de-recherche-du-quebec-sante"
CKAN_API     = f"https://www.donneesquebec.ca/recherche/api/3/action/package_show?id={DATASET_SLUG}"
FUND_LABEL   = "FRQS"   # the value in the Fonds column we expect

_WS = re.compile(r"\s+")

def fetch_resources():
    """Return [(year_label, url)] for each CSV resource with non-zero size."""
    req = urllib.request.Request(CKAN_API, headers={"User-Agent": "OpenAlex-FRQS/1.0 (contact@openalex.org)"})
...
def download_all(output_dir: Path):
    ...
    print(f"[1/2] Downloading {len(resources)} FRQS CSVs from CKAN")
    dfs = []
    for year, url in resources:
        print(f"      {year}: {url}")
        p = output_dir / f"frqs_{year}.csv"
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter to FRQS-funded rows only (the Fonds column should always be FRQS for this dataset,
    # but cofunded rows sometimes list co-funders; we keep all rows that have Fonds==FRQS or empty).
    ...
    out["provenance"]    = "frqs_data_quebec"
...
def main():
    ap = argparse.ArgumentParser(description="FRQS (donneesquebec.ca CKAN) -> S3")
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/frqs"))
    ap.add_argument("--skip-upload", action="store_true")
    ap.add_argument("--allow-shrink", action="store_true")
    a = ap.parse_args()
    a.output_dir.mkdir(parents=True, exist_ok=True)
    print("=" * 60)
    print("FRQS (Fonds de recherche du Québec - Santé) -> S3")
    print("=" * 60)
    src = download_all(a.output_dir)
    df = transform(src)
    out = a.output_dir / "frqs_projects.parquet"
    ...
    print(f"\nNext: notebooks/awards/CreateFRQSAwards.ipynb in Databricks")
