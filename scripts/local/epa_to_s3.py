#!/usr/bin/env python3
"""
EPA Awards to S3 Data Pipeline (US Environmental Protection Agency)
================================

This script downloads EPA grant data from USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/epa/epa_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for EPA grants (research-funding component, e.g. STAR program) (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/epa/

Usage:
    python epa_to_s3.py

    # Resume interrupted download:
    python epa_to_s3.py --resume

    # Skip upload to S3:
    python epa_to_s3.py --skip-upload
...
# EPA agency info
EPA_AGENCY_NAME = "Environmental Protection Agency"
...
S3_KEY = "awards/epa/epa_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for EPA grants (research-funding component, e.g. STAR program) in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "epa_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download EPA grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"epa_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download EPA grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading EPA grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {EPA_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-EPA-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("epa_fy*.zip"))
...
def main():
    parser = argparse.ArgumentParser(
        description="Download EPA grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./epa_data"),
        help="Directory for downloaded/processed files (default: ./epa_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("EPA Awards to S3 Data Pipeline (US Environmental Protection Agency)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("epa_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateEPAAwards.ipynb")
