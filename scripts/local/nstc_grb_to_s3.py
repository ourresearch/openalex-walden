#!/usr/bin/env python3
"""
Taiwan NSTC GRB projects -> S3 pipeline
=======================================

Downloads current National Science and Technology Council (Taiwan) project
records from the public Government Research Bulletin (GRB) search/export API
...
This ingest is scoped to the current NSTC plan-organ code only:

    BT200 = 國家科學及技術委員會(本會)

GRB also contains legacy Ministry of Science and Technology (BT100) and
National Science Council (BT00) records. Those map to different OpenAlex
funder entities and should be ingested separately if pursued.

Validation on 2026-06-02 source probe:
  - BT200 search returned 56,723 records.
...
Output
------
s3://openalex-ingest/awards/nstc_grb/nstc_grb_projects.parquet

Usage
-----
    python nstc_grb_to_s3.py --limit 10 --skip-upload
    python nstc_grb_to_s3.py --skip-upload
    python nstc_grb_to_s3.py --skip-download --skip-upload
    python nstc_grb_to_s3.py --allow-shrink
...
FUNDER_DISPLAY_NAME = "National Science and Technology Council"
...
PLAN_ORGAN_CODE = "BT200"
PLAN_ORGAN_NAME = "國家科學及技術委員會(本會)"

S3_BUCKET = "openalex-ingest"
S3_KEY = "awards/nstc_grb/nstc_grb_projects.parquet"

USER_AGENT = "openalex-walden-nstc-grb-ingest/1.0 (+https://openalex.org)"
ALLOWED_PAGE_SIZES = (10, 50, 100, 200)
DEFAULT_PAGE_SIZE = 200
EXPECTED_MIN_FULL_ROWS = 44000
...
def fetch_exports(output_dir: Path, limit: Optional[int]) -> tuple[list[Path], int]:
    ...
    if total <= 0:
        raise RuntimeError("GRB search returned zero NSTC rows.")
    expected_rows = min(limit, total) if limit else total
    expected_pages = math.ceil(expected_rows / page_size)
    log(f"GRB reports {total:,} BT200/NSTC records; fetching {expected_rows:,} rows across {expected_pages:,} pages.")
    ...
    def handle_page(page: int, payload: dict[str, Any], remaining: Optional[int]) -> None:
        ...
        if not ids:
            return
        export_path = export_dir / f"nstc_grb_page_{page:05d}.xlsx"
    ...
    (output_dir / "nstc_grb_manifest.json").write_text(json.dumps(manifest, ensure_ascii=False, indent=2))
...
def load_cached_exports(output_dir: Path, limit: Optional[int]) -> tuple[list[Path], int]:
    ...
    print("=" * 60)
    export_dir = output_dir / "raw_exports"
    exports = sorted(export_dir.glob("nstc_grb_page_*.xlsx"))
    if not exports:
    ...
    manifest_path = output_dir / "nstc_grb_manifest.json"
    reported_total = 0
...
def normalize_dataframe(raw: pd.DataFrame, reported_total: int, limit: Optional[int]) -> pd.DataFrame:
    ...
    for _, row in raw.iterrows():
        ...
        rows.append({
            ...
            "grb_id": grb_id,
            "display_name": title_en or title_zh or f"NSTC project {funder_award_id}",
            "title_zh": title_zh,
            "title_en": title_en,
...
def aggregate_awards(df: pd.DataFrame) -> pd.DataFrame:
    """Aggregate GRB annual/period rows to one award row per cited NSTC grant number."""
    if df.empty:
        return df
    before = len(df)
...
def validate_dataframe(df: pd.DataFrame, *, full_run: bool) -> None:
    ...
    if full_run and total < EXPECTED_MIN_FULL_ROWS:
        raise RuntimeError(
            f"Full NSTC GRB run returned only {total:,} rows; expected at least {EXPECTED_MIN_FULL_ROWS:,}."
        )
...
def write_outputs(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    parquet_path = output_dir / "nstc_grb_projects.parquet"
    df = df.astype("string")
...
def check_no_shrink(new_count: int, allow_shrink: bool, output_dir: Path) -> bool:
    ...
    prev_path = output_dir / "_prev_nstc_grb_projects.parquet"
    try:
    ...
    if new_count < prev_count:
        if allow_shrink:
            ...
            return True
        print(f"\n[ERROR] Refusing to shrink NSTC GRB corpus ({prev_count:,} -> {new_count:,}).")
        return False
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Download Taiwan NSTC GRB project records and write OpenAlex staging parquet.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
    )
    parser.add_argument("--output-dir", type=Path, default=Path("/tmp/nstc_grb"))
    ...
    print("=" * 60)
    print("Taiwan NSTC GRB projects -> S3 pipeline")
    print("=" * 60)
