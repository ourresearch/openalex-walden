#!/usr/bin/env python3
"""
AHRQ Awards to S3 Data Pipeline (US Agency for Healthcare Research and Quality, HHS subtier)
================================

This script downloads AHRQ grant data from USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/ahrq/ahrq_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for AHRQ grants (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/ahrq/

Usage:
    python ahrq_to_s3.py

    # Resume interrupted download:
    python ahrq_to_s3.py --resume

    # Skip upload to S3:
    python ahrq_to_s3.py --skip-upload
...
# AHRQ agency info
AHRQ_AGENCY_NAME = "Agency for Healthcare Research and Quality"
...
S3_KEY = "awards/ahrq/ahrq_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for AHRQ grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "ahrq_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download AHRQ grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"ahrq_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download AHRQ grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading AHRQ grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {AHRQ_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-AHRQ-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("ahrq_fy*.zip"))
...
def save_to_parquet(df: pd.DataFrame, output_dir: Path) -> Path:
    ...
    Returns:
        Path to output file
    """
    output_path = output_dir / "ahrq_awards.parquet"
...
def main():
    parser = argparse.ArgumentParser(
        description="Download AHRQ grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./ahrq_data"),
        help="Directory for downloaded/processed files (default: ./ahrq_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("AHRQ Awards to S3 Data Pipeline (US Agency for Healthcare Research and Quality, HHS subtier)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("ahrq_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateAHRQAwards.ipynb")
