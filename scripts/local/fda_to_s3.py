#!/usr/bin/env python3
"""
FDA Awards to S3 Data Pipeline (US Food and Drug Administration, HHS subtier — regulatory science research grants)
================================

This script downloads FDA grant data from USAspending.gov API,
processes it into a parquet file, and uploads it to S3 for Databricks ingestion.

Data Source: https://api.usaspending.gov/
Output: s3://openalex-ingest/awards/fda/fda_awards.parquet

What this script does:
1. Requests bulk downloads from USAspending API for FDA grants (FY2001-2025)
...
Requirements:
    ...
    s3://openalex-ingest/awards/fda/

Usage:
    python fda_to_s3.py

    # Resume interrupted download:
    python fda_to_s3.py --resume

    # Skip upload to S3:
    python fda_to_s3.py --skip-upload
...
# FDA agency info
FDA_AGENCY_NAME = "Food and Drug Administration"
...
S3_KEY = "awards/fda/fda_awards.parquet"
...
def request_bulk_download(year: int, session: requests.Session) -> dict:
    """
    Request a bulk download for FDA grants in a specific fiscal year.
...
class ProgressTracker:
    ...
    def __init__(self, output_dir: Path):
        ...
        self.checkpoint_file = output_dir / "fda_checkpoint.json"
        self.data = {
            "completed_years": [],
            "failed_years": [],
            "total_rows": 0,
...
def download_year(
    ...
    """
    Download FDA grants for a single fiscal year.
    ...
    """
    zip_path = output_dir / f"fda_fy{year}.zip"
...
def download_all_years(
    ...
    """
    Download FDA grants for all fiscal years.
    ...
    """
    print(f"\n{'='*60}")
    print("Step 1: Downloading FDA grants from USAspending")
    print(f"{'='*60}")
    print(f"  Agency: {FDA_AGENCY_NAME}")
    ...
    session.headers.update({
        ...
        "User-Agent": "OpenAlex-FDA-Ingest/1.0"
    })
    ...
    return sorted(output_dir.glob("fda_fy*.zip"))
...
def main():
    parser = argparse.ArgumentParser(
        description="Download FDA grants from USAspending and upload to S3"
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("./fda_data"),
        help="Directory for downloaded/processed files (default: ./fda_data)"
    )
    parser.add_argument(
        "--resume",
        action="store_true",
    ...
    print("=" * 60)
    print("FDA Awards to S3 Data Pipeline (US Food and Drug Administration, HHS subtier — regulatory science research grants)")
    print("=" * 60)
    ...
    if not args.skip_download:
        ...
        zip_files = sorted(args.output_dir.glob("fda_fy*.zip"))
    ...
    print(f"{'='*60}")
    print(f"\nNext step:")
    print(f"  In Databricks, run: notebooks/awards/CreateFDAAwards.ipynb")
