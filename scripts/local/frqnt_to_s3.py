#!/usr/bin/env python3
"""
FRQNT (Fonds de recherche du Québec - Nature et technologies) to S3 Data Pipeline
==================================================================================

Downloads the official FRQNT funded-projects CSVs from donneesquebec.ca CKAN and
writes a parquet for Databricks.

Data Source:
  CKAN: https://www.donneesquebec.ca/recherche/api/3/action/package_show?id=liste-du-financement-accorde-par-le-fonds-de-recherche-du-quebec-nature-et-technologies
...
Output: s3://openalex-ingest/awards/frqnt/frqnt_projects.parquet

Schema notes:
  - funder_award_id = Dossier (the FRQNT internal project ID, e.g. 196577 — this
    is the integer cited in OpenAlex grants links to FRQNT and matches the
    'Numéro de dossier' shown on FRQNT award letters).
...
Usage:
    python frqnt_to_s3.py --skip-upload
"""
...
S3_KEY    = "awards/frqnt/frqnt_projects.parquet"
DATASET_SLUG = "liste-du-financement-accorde-par-le-fonds-de-recherche-du-quebec-nature-et-technologies"
CKAN_API     = f"https://www.donneesquebec.ca/recherche/api/3/action/package_show?id={DATASET_SLUG}"
FUND_LABEL   = "FRQNT"   # the value in the Fonds column we expect

_WS = re.compile(r"\s+")

def fetch_resources():
    """Return [(year_label, url)] for each CSV resource with non-zero size."""
    req = urllib.request.Request(CKAN_API, headers={"User-Agent": "OpenAlex-FRQNT/1.0 (contact@openalex.org)"})
...
def download_all(output_dir: Path):
    ...
    print(f"[1/2] Downloading {len(resources)} FRQNT CSVs from CKAN")
    dfs = []
    for year, url in resources:
        print(f"      {year}: {url}")
        p = output_dir / f"frqnt_{year}.csv"
...
def transform(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Filter to FRQNT-funded rows only (the Fonds column should always be FRQNT for this dataset,
    # but cofunded rows sometimes list co-funders; we keep all rows that have Fonds==FRQNT or empty).
    ...
    out["provenance"]    = "frqnt_data_quebec"
...
def main():
    ap = argparse.ArgumentParser(description="FRQNT (donneesquebec.ca CKAN) -> S3")
    ap.add_argument("--output-dir", type=Path, default=Path("/tmp/frqnt"))
    ap.add_argument("--skip-upload", action="store_true")
    ap.add_argument("--allow-shrink", action="store_true")
    a = ap.parse_args()
    a.output_dir.mkdir(parents=True, exist_ok=True)
    print("=" * 60)
    print("FRQNT (Fonds de recherche du Québec - Nature et technologies) -> S3")
    print("=" * 60)
    src = download_all(a.output_dir)
    df = transform(src)
    out = a.output_dir / "frqnt_projects.parquet"
    ...
    print(f"\nNext: notebooks/awards/CreateFRQNTAwards.ipynb in Databricks")
