...
def repo_enriched():
    ...
    endpoint_lookup = (
        ...
        .withColumnRenamed("endpoint_id", "_lookup_endpoint_id")
    )
    combined_df = (
        combined_df
        .join(endpoint_lookup, "native_id", "left")
        .withColumn("endpoint_id", F.coalesce(F.col("endpoint_id"), F.col("_lookup_endpoint_id")))
        .drop("_lookup_endpoint_id")
    )
