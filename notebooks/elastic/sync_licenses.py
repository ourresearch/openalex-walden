...
if spark.catalog.tableExists("openalex.common.licenses_api"):
    spark.sql("""
        CREATE OR REPLACE TABLE openalex.common.licenses_api_hash AS
        ...
        FROM openalex.common.licenses_api
    """)
else:
    spark.sql("""
        CREATE TABLE IF NOT EXISTS openalex.common.licenses_api_hash (
...
# MAGIC %sql
# MAGIC -- Rebuild licenses_api table
# MAGIC CREATE OR REPLACE TABLE openalex.common.licenses_api AS
# MAGIC WITH license_counts AS (
# MAGIC     SELECT
# MAGIC         license,
...
# MAGIC             location.license as license
# MAGIC         FROM openalex.works.openalex_works
# MAGIC         LATERAL VIEW explode(locations) as location
# MAGIC         WHERE location.license IS NOT NULL
...
# MAGIC JOIN license_counts lc on l.id = lc.license
...
# MAGIC     FROM openalex.common.licenses_api
# MAGIC )
# MAGIC MERGE INTO openalex.common.licenses_api AS target
# MAGIC USING (
# MAGIC     SELECT n.id,
# MAGIC         CASE
...
# MAGIC     LEFT JOIN openalex.common.licenses_api_hash p ON n.id = p.id
# MAGIC ) AS source
...
CONFIG = {
    "table_name": "openalex.common.licenses_api",
