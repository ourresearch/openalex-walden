...
if spark.catalog.tableExists("openalex.common.work_types_api"):
    spark.sql("""
        CREATE OR REPLACE TABLE openalex.common.work_types_api_hash AS
        ...
        FROM openalex.common.work_types_api
    """)
else:
    spark.sql("""
        CREATE TABLE IF NOT EXISTS openalex.common.work_types_api_hash (
...
# MAGIC %sql
# MAGIC -- Rebuild work_types_api table
# MAGIC CREATE OR REPLACE TABLE openalex.common.work_types_api AS
# MAGIC WITH work_types_counts as (
# MAGIC     SELECT
...
# MAGIC       AND type != 'grant'
# MAGIC     GROUP by type
# MAGIC )
# MAGIC SELECT
# MAGIC     wt.id as original_id,
...
# MAGIC     wtc.cited_by_count,
# MAGIC     DATE_TRUNC('SECOND', wt.created_date) AS created_date,
# MAGIC     CAST(NULL AS TIMESTAMP) AS updated_date
# MAGIC FROM openalex.common.work_types wt
# MAGIC JOIN work_types_counts wtc ON wt.id = wtc.type
...
# MAGIC     FROM openalex.common.work_types_api
# MAGIC )
# MAGIC MERGE INTO openalex.common.work_types_api AS target
# MAGIC USING (
# MAGIC     SELECT n.id,
# MAGIC         CASE
...
# MAGIC     LEFT JOIN openalex.common.work_types_api_hash p ON n.id = p.id
# MAGIC ) AS source
...
CONFIG = {
    "table_name": "openalex.common.work_types_api",
