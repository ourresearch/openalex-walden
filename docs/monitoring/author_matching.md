...
> Every writer (MatchAuthors, UpdateWorkAuthors, AuthorshipDailyMetrics, the judge) emits to openalex.monitoring.metrics; history before 2026-09-15 was copied in from the retired openalex.authors.authorship_daily_metrics (oxjob 640). Metrics born 2026-09-13 (stage_*, e2e_runs_on_date, block_skew, seats_by_*, assignment_log_rows) have absolute rules only until ~2026-09-27 (min_history). author_matching_sql did not land on 09-13 and wrote no collector_error — fix before its checks mean anything.
...
| **wall time per authorship stage** `stage_wall` | `stage_wall_sec[*]` | relative: 3/6 MAD, up only, 14d window  (max watch) |  |
| **SQL cost of the matching pass** `sql_cost` | `author_matching_sql[*]` | relative: 3/6 MAD, up only, 14d window  (max watch) | one hot block → one task with 2.5B rows; metric MISSING on 09-13, collector needs a fix |
| **monitor collected everything** `collector_errors` | `sum(collector_error[*])` | absolute: watch if > 0 | Jobs API / query-history permission changes; ephemeral tables missing at read time |
...
| **matches per tier (catches a tier going silent)** `tier_volume` | `match_tier[*]` | relative: 3/6 MAD, 14d window  (dimensions from window, max watch) | n3/n4/n5 never fire — known state, not this check |
...
| **reservoir filled vs added (eligible)** `reservoir_draining` | `(null_reservoir_flow[eligible_added_existing] + null_reservoir_flow[eligible_added_new_works]) / null_reservoir_flow[eligible_filled]` | relative: 3/6 MAD, up only, 14d window | matcher skipping the reservoir; eligibility gate (work_id > 7e9 / created ≥ 2025-12-20) drift |

## Are the matches we make correct?

| check | expression | rule | known failure modes |
|---|---|---|---|
| **judged precision by tier (7-day rolling)** `precision_by_tier` | `judge_arm_a[*\|same_person] / (judge_arm_a[*\|same_person] + judge_arm_a[*\|different_person])` | absolute: critical if < 0.7, watch if < 0.8  (7-day rolling, n ≥ 30, **pages**) | n8 tiers at 6–26% (cut 07-28); s5_n2 at 28–33% on wave nights = contamination, not the tier — see precision_wave_excluded |
| **judged precision by tier, excluding hot-name seats** `precision_wave_excluded` | `judge_arm_a_clean[*\|same_person] / (judge_arm_a_clean[*\|same_person] + judge_arm_a_clean[*\|different_person])` | absolute: critical if < 0.7, watch if < 0.8  (7-day rolling, *proposed, not evaluated*) | needs a rollup joining authorship_daily_quality_sample to name_concentration_top (open decision 1 in the oxjob) |
| **ORCID-tier judged precision (7-day rolling)** `precision_orcid` | `judge_arm_a[orcid\|same_person] / (judge_arm_a[orcid\|same_person] + judge_arm_a[orcid\|different_person])` | absolute: watch if < 0.9  (7-day rolling, n ≥ 30) | PNNL staff ORCIDs on data-deposit records (36/78 all-time FPs from S7407051155) — exclude that source before trusting a dip |
...
| **name-change guard verdicts** `guard_verdicts` | `guard_telemetry[*]` | relative: 3/6 MAD, 14d window  (max watch) | oxjob 608 misbinding class; guard blocking everything is its own regression |
...
| **impossible name on a new assignment (seat name cannot be the profile it was bound to)** `impossible_name` | `assign_name_check[*]` | relative: 3/6 MAD, up only, 14d window  (**pages**) | 09-13 over 509K judged seats — foreign_family 454 (26 name-tier / 428 ORCID), first_initial_clash 358 (74 / 284). ORCID matches have NO name gate; this is their only one. A name-tier hit means the profile's display name drifted since the match. Most first-initial clashes are parser noise (middle-name-as-first: "Washington, Todd" → "M. Todd Washington"); the metric already requires the seat's given name to be absent from the profile name. |
| **foreign-family assignments, all tiers** `impossible_name_total` | `sum(assign_name_check[foreign_family\|*])` | absolute: critical if > 1000  (**pages**) |  |
...
| **mint volume (informational)** `mint_volume` | `new_authors_minted` | relative: 3/6 MAD, 14d window  (max watch) | baseline 30–70K; 90–120K on big-batch days |
...
| check | expression | rule | known failure modes |
|---|---|---|---|
| **hot names (>1K seats/batch) and seats in them** `hot_names` | `name_concentration[*]` | relative: 3/6 MAD, up only, 14d window  (max watch) | Geoscience Australia 43K/night, Scan-the-World, GBIF.org User, Atlas of Living Australia ×5 |
| **single name over the wave line** `hot_name_max` | `name_concentration_top[*]` | absolute: critical if > 100000, watch if > 25000  (**pages**) |  |
...
| **seats in blocks over 100K** `seats_in_huge_blocks` | `block_skew[seats_in_blocks_gt100k]` | relative: 3/6 MAD, up only, 14d window  (max watch) |  |
| **one source's share of the batch (top 20)** `source_share` | `seats_by_source[*] / sum(match_outcome[*])` | absolute: watch if > 0.3; relative: 3/6 MAD, up only, 14d window | S7407051155 (DOE PNNL) mints 20–24K dataset works/day; S7407053303 IGSN samples |
| **matched share within a hot source (bulk-binding an org roster)** `hot_source_matched_share` | `matched_by_source[*] / seats_by_source[*]` | absolute: watch if > 0.9  (max watch) | staff ORCIDs on deposit records bulk-bind to publication profiles |
...
| **seats added to existing works** `seats_added` | `author_list_seats_added` | relative: 3/6 MAD, 14d window  (max watch) |  |
| **seats removed from existing works** `seats_removed` | `author_list_seats_removed` | relative: 3/6 MAD, 14d window  (max watch) |  |
...
| **stale-seat backlog and works missing seats** `stale_backlog` | `stale_seats[*]` | relative: 3/6 MAD, up only, 14d window  (max watch) |  |
| **name-string drift works (#608 guard staging)** `string_drift` | `string_drift_works` | relative: 3/6 MAD, 14d window  (max watch) |  |
...
| **judge cost (cents)** `judge_cost` | `judge_cost_cents` | absolute: critical if > 5000, watch if > 1000 | double pass (07-26/27); arm A 25→50 = +$1.50; baseline ~320 |
...
1. precision_wave_excluded needs a rollup joining authorship_daily_quality_sample to the hot-name list (judge_arm_a_clean). Until then, precision_by_tier will flag s5_n2 on every wave night.
2. Precision thresholds (80/70, 90 for ORCID) come from the healthy-tier band in the oxjob 640 history, not from a product target. Replace with the stated author-precision goal if one exists.
3. Which items page. Draft — sections run, correctness (impossible names), waves, plus the reservoir/stale/fingerprint absolutes. Everything else is digest-only until the false-positive rate is known (14 nights observation-only).
4. assign_name_check ships in the MatchAuthors tail with the oxjob 608 predicate inlined; lifting it into a shared SQL UDF is a follow-up once it has a second caller.
