...
| **mints whose ORCID an older profile already held** `mint_dup_holder` | `new_authors_minted[dup_of_existing_holder]` | absolute: critical if > 1000, watch if > 50  (**pages**) | the ORCID tier's holder lookup is stale or wrong: 2026-07-15/16 minted 59K same-ORCID profiles in two nights unseen. Before oxjob #444 the 12:21 UTC authors_for_matching rebuild racing this run leaked ~40/night; the tier now also reads openalex.authors.authors, so expect ~0. |
