...
SELECT
  ...
  -- Objection: orphan funder ids (present in source, absent from openalex.funders.funders) — these
  ...
  (SELECT COUNT(*) FROM (SELECT DISTINCT c.funder_id
     ...
     LEFT ANTI JOIN openalex.funders.funders f ON f.funder_id = c.funder_id)) AS candidate_orphan_funder_ids,
  (SELECT COUNT(*) FROM (SELECT DISTINCT cwf.funder_id
     FROM openalex.awards.crossref_work_funders cwf
     LEFT ANTI JOIN openalex.funders.funders f ON f.funder_id = cwf.funder_id)) AS crossref_orphan_funder_ids;
