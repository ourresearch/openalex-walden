...
CRITICAL (learned 2026-08-05): authors_for_matching is rebuilt daily at 12:00 UTC
by the authors job and then contains the SAME MORNING's mints. Any sim run after
noon UTC must pin AFM to the decision-time version or ambiguous seats "match"
their own minted twins and every diff class inflates:
    import ntierfix_validate as m
    m.AFM = "openalex.authors.authors_for_matching VERSION AS OF <pre-noon version>"
Old-logic counterfactual: m.nf = lambda col: col.
"""
import json
import sys
import time
