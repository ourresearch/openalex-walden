...
# --------------------------------------------------------------------------- #
# oxjob #881: the repair_mojibake necessary-condition gate must be EXACT.

def test_gate_ascii_is_untouched_fast_path():
    s = "A plain ASCII abstract with entities-free text and no tags, 42% of the corpus."
    assert tc.repair_mojibake(s) is s  # identity, not just equality: the fast path returns s


def test_gate_never_skips_repairable_text():
    # every known-repairable fixture from this suite must still repair identically
    cases = [
        "UniversitÃ© de MontrÃ©al",
        "Universit&Atilde;&copy;",       # entity-encoded mojibake (repairs after decode)
        "MuÌˆnster",                      # NFD combining-mark mojibake shape
    ]
    for s in cases:
        decoded = tc.decode_entities(s)
        if tc.repair_mojibake(decoded) != decoded:
            # it repairs -> the gate must NOT have skipped it
            assert decoded.isascii() is False
            assert tc._TRIGGER_LEAD_RE.search(decoded) is not None


def test_gate_trigger_class_covers_detector_leads():
    # the gate is derived from the same primitives as the detectors; if someone widens
    # _CORE_LEADS_2 without the gate picking it up, this fails
    for ch in tc._CORE_LEADS_2 | {"â"}:
        assert tc._TRIGGER_LEAD_RE.search(ch), ch
    for cp in list(range(0xE0, 0xF0)) + list(range(0xF0, 0xF5)):
        if cp == 0xE2:
            continue
        assert tc._TRIGGER_LEAD_RE.search(chr(cp)), hex(cp)


def test_accented_but_clean_text_unchanged():
    # é is inside the trigger range (a 3-byte lead candidate) so it IS scanned -- and must
    # come through unchanged since there is no continuation pair
    s = "Vulnérabilité des systèmes: étude à Montréal"
    assert tc.repair_mojibake(s) == s
    assert tc.clean_text(s) == s
